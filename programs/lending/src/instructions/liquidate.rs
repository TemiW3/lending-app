...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> {
    ...
    if health_factor >= 1.0 {
        return Err(ErrorCode::HealthFactorAboveThreshold.into());
    }
    ...
    let liquidation_amount = total_borrowed.checked_mul(collateral_bank.liquidation_close_factor).unwrap();
    ...
    let cpi_ctx_to_liquidator = CpiContext::new_with_signer(cpi_program, transfer_to_liquidator, signer_seeds);
